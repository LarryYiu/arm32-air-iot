...
{
    // float temp     = SHT20_GetTemp(true);
    // float humidity = SHT20_GetHumidity(true);
    // if(!isnan(temp))
    // {
    //     int32_t temp = (int32_t)(SHT20_GetTemp(true) * 100.0f);
    //     DBG_log("[SHT20] Temp: %d.%02d C\n", temp / 100, ((temp % 100) > 0 ? abs(temp % 100) : 0));
    // }

    // if(!isnan(humidity))
    // {
    //     int32_t humidity = (int32_t)(SHT20_GetHumidity(true) * 100.0f);
